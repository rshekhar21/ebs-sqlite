...
FROM
    ...
    LEFT JOIN (
        SELECT o.`order_date`, SUM(l.`net`) AS `net`, SUM(l.`qty`) AS `qty`
        FROM `sold` l
    ...
    LEFT JOIN (
        SELECT o.`order_date`, SUM(l.`gross`) AS `gross`, SUM(l.`qty`) AS `qty`
        FROM `sold` l
            LEFT JOIN `orders` o ON l.`order_id` = o.`id`        
        GROUP BY
            o.`order_date`
    ) `gsl` ON gsl.`order_date` = o.`order_date`
    LEFT JOIN (
